...
Some fields in CollectionSpace are controlled. That is, you can only populate them using terms/values that are already established in a controlled vocabulary.

This page describes the three types of controlled vocabularies in CollectionSpace.
...
* Users with a sufficient level of permissions can create, edit, and delete authority records in the CollectionSpace web application
* You can search for and export data from authorities in the CollectionSpace web application
* Hierarchical relationships can be created between authority terms in the same authority vocabulary
* The Merge Authority Items data update merges two authority terms from the same authority vocabulary. All reference to the source term are transferred to the target term.
...
* Users with a sufficient level of permissions can create, edit, and delete terms in the CollectionSpace web application under `Tools > Term Lists`.
...
* Defined in your CollectionSpace instance's UI configuration
* For Lyrasis hosted clients, you must put in a Support Desk ticket requesting additions/changes to these. We will ask xref:option_list_value_change_options.adoc[which approach you want to take for any changes]. The faster/easier approach can take a week or longer, since multiple people are involved in changing your UI configuration code, reviewing and merging those changes, and redeploying your CollectionSpace instance.
...
If you are running your own CollectionSpace instance, you can customize such things, but doing so can significantly complicate your future upgrade/maintenance path.

We do not offer this type of customization for Lyrasis-hosted CollectionSpace clients, as it is unsustainable to support.
