...
Some clients migrate to CollectionSpace from systems where precoordinate subject indexing systems like Library of Congress Subject Headings were used.

These clients will need to decide how they want this subject data treated in CollectionSpace.
...
== What works best in CollectionSpace?
...
* Your needs and priorities for using this data in CollectionSpace
* To some extent, what CollectionSpace domain profile you are migrating into

Below, we'll go over some pros and cons of each of the approaches described above in the context of CollectionSpace.

== Pros and cons of approaches

=== Faceted postcoordinate

==== PRO: In general, the design and current functionality of CollectionSpace appears to suggest this as the preferred approach

The fields in the CollectionSpace object record's _Object Description Information > Content_ and _Object History and Association Information > Associations_ sections support this approach:
...
Taking this approach, we create the necessary terms in the appropriate authority vocabularies and populate the record like this:footnote:[As of v7.2, "20th century" would actually go in the new, controlled _Content era/period_ field. The time and effort to redo this screenshot is not worth the benefit to explaining the overall concepts of this page.]
...
[TIP]
====
Given your specific data and the domain profile you are migrating into, your Migration Specialist will be able to tell you how this CON would impact you.
====
...
[NOTE]
====
With this approach, the review and cleanup required prior to loading any data at all into CollectionSpace can take quite a long time, since:
...
Since all the terms will be going into the same authority vocabulary, you can merge any variants within CollectionSpace using the Merge Authority tool.
...
Release 8.3's support for `contains` or `matches` (with wildcard) searches of authority-controlled field values mitigates this somewhat.
...
==== CON: CollectionSpace does not support this approach
