...
** In the Local History and Material Culture domain profile, these fields are renamed `Home location/building` and `Temp location/building`
** In the Botanical Garden domain profile, the main location field is renamed `Garden location` field, and there is no normal or home location field.
