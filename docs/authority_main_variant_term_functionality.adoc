...
[NOTE]
====
If you are *not* a Lyrasis-hosted CollectionSpace user with a standard service contract, you _may_ be able to select the variant term for use in your record:
...
**We recommend configuring your CollectionSpace instance to enforce the use of main terms only.** If you are a Lyrasis-hosted CollectionSpace user who does not have a special service agreement, this is already enforced for you. See the https://github.com/lyrasis/collectionspace-migration-explainers/blob/main/docs/authority_main_variant_term_functionality.adoc#data-integrity-uniqueness-and-batch-data-processing-issues[Data integrity, uniqueness, and batch data processing issues] section for reasoning. You will still be able to type "Prince" to bring up "Wonder Woman," but you will only be able to assign "Wonder Woman" in the record.
====
...
You can change the value of a Display name in an authority record. When you save the authority record, all fields populated with that display name are updated behind the scenes to display the new value.

[WARNING]
====
Note that this can take a very long time if the display name has been used in large numbers of fields.

If a term has been updated more than ~9500 times, the application may time out while attempting to update the usages.
====

=== Merging authorities

This data update functionality is documented in the https://collectionspace.atlassian.net/wiki/spaces/CUD/pages/3435036673/Authority+Control#Merging-Authorities[CollectionSpace User Manual 🔒].
...
[WARNING]
====
This can also take a very long time or time out if the term being merged into the target term is used in a large number of records.
====
...
* The entire point of authorities/controlled vocabularies is to establish one main form of name for each entity or concept, to maximize consistency in data. Allowing the assignment of variant names in records subverts the purpose of having authorities.
* Currently, you cannot round-trip data using variant terms by exporting CSV data, making updates, and re-ingesting with the CSV Importer. The CSV Importer cannot match/insert variant names into CollectionSpace records via batch processing. Adding the ability to do this is not currently on the roadmap for development.
...
* If using the public browser, variant terms will facet differently/separately from the main terms. Again, this goes against the spirit of authorities, which are implemented in order to enforce data consistency
...
The user interface makes it possible for a careful human entering data to differentiate between these two names and select the appropriate value when entering data by hovering over each entry in the search list to see more details:
...
Both our "Anonymous" names would get lumped into one "Anonymous" faceted/linked name value.
...
[TIP]
====
You can prepare your CSVs with the refnames instead of `termDisplayName` strings.

You would enter the refname value(s) in a `objectProductionPersonRefname` column and get rid of the `objectProductionPersonPersonLocal` and `objectProductionPersonPersonUlan` columns.

*But this is not end-user friendly!*
====

Usually, you prepare CSVs for ingest by entering the text of the Display name ("Anonymous") into the column for the target field.
...
Because migration processes are use batch data ingest tools, the data we migrate is under more constraints than data you might manually enter into CollectionSpace:
...
* Configure your CollectionSpace instance to disallow populating fields with variant display names for terms (given examples below, this means populating all fields with "Wonder Woman" and not using "Diana Prince" in records). This option is turned OFF for Lyrasis hosted clients.
* Do your best to maintain unique main display names in a given authority vocabulary (i.e. Person/Local or Organization/ULAN). If you are a Lyrasis hosted CollectionSpace client, we can quickly run a report of duplicate terms in a given authority for you on request.
