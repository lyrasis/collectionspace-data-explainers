...
Object Valuation Report:: Documented in https://collectionspace.atlassian.net/wiki/spaces/COL/pages/2672361473/Report+Details+Object+Valuation+DRAFT+WORK+IN+PROGRESS[the CollectionSpace Report Repository]. This report runs on a list of selected objects (search results list or list of objects related to a procedure). It gives you a spreadsheet with some very basic identifying info about each object, and information from the most recent valuation control procedure related to each object.
