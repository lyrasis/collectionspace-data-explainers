...
What's going on with Obj 2? The most recent VC related to it is VC LI 1. This VC is related to 3 objects, so the report calculates and returns the average value per object. (It does let you filter to objects related to VCs related to more than one object, so you can tell which are calculated values). 
